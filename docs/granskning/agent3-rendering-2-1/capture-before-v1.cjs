...
async function main() {
  ...
  const mailCases = [
    ['mail-invoice', 'sendInvoice', { pdfBuffer: pdfs.get('invoice-customer-classic') }],
    ...
    ['mail-notice', 'sendRentNotice', { amount: 9000, pdfBuffer: pdfs.get('notice-rent') }],
  ...
  const manifest = { baseSha: renderingFixtureBaseSha, sourceHead: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(),
