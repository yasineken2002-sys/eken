...
// Independent pin computed from that commit's ordered Git tree, never from a capture at runtime.
const FROZEN_SOURCE_LIST_SHA256 = '75603a26e9273fcc9857cfef13390c2803eb3bbd5b50fd41b1af936f82d5fa58'
...
function sourcesAtBase() {
  ...
  const sources = tree.split('\0').filter(Boolean).map(line => {
  ...
  verifySourceList(sources.map(source => ({ path: source.path, baseGitBlob: source.blob })))
  return sources
}

function verifySourceList(sources) {
  const list = sources.map(({ path, baseGitBlob }) => ({ path, baseGitBlob }))
  assert.equal(digest(JSON.stringify(list)), FROZEN_SOURCE_LIST_SHA256, 'Exact pinned frozen source list')
}

function readArtifact(directory, metadata) {
  assert.ok(!metadata.file.split(/[\\/]/).includes('..'), 'Artifact must remain in its capture')
  const file = resolve(directory, metadata.file)
  assert.ok(file.startsWith(resolve(directory) + sep), 'Artifact must remain in its capture')
...
// Explicit source archive for after captures; missing/changed evidence is an error, never a Git fallback.
function sourcesFromArchive() {
  const directory = join(evidenceRoot, 'extra-mail-before-utc')
  const manifest = JSON.parse(readFileSync(join(directory, 'manifest.json'), 'utf8'))
  assert.equal(manifest.mode, 'before')
  assert.equal(manifest.baseSha, BASE_SHA)
  assert.equal(manifest.applicationSourceSha, BASE_SHA)
  verifySourceList(manifest.sourceBlobs)
  return manifest.sourceBlobs.map(source => {
    assert.equal(source.source.file, 'sources/' + source.path)
    const bytes = readArtifact(directory, source.source)
    assert.equal(source.actualGitBlob, source.baseGitBlob)
    assert.equal(gitBlob(bytes), source.baseGitBlob, source.path + ': exact archived frozen blob')
    return { path: source.path, blob: source.baseGitBlob, bytes }
  })
}
...
async function capture(mode, directory) {
  ...
  const frozen = mode === 'before' ? sourcesAtBase() : sourcesFromArchive()
...
function readExtraMailCapture(directory) {
  ...
  assert.equal(manifest.baseSha, BASE_SHA)
  assert.ok(['before', 'after'].includes(manifest.mode), 'Known capture mode')
  if (manifest.mode === 'before') assert.equal(manifest.applicationSourceSha, BASE_SHA)
  const artifact = metadata => readArtifact(directory, metadata)
