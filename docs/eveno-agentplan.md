...
| 7 | G2 delegationer + "Gör alltid detta" + preferenser | 6 | hyresvärden kan delegera och se vad systemet tror om hen | **DELVIS** `df9e657` — **delegationsdelen är hel**: modellen och grinden ([#803](https://github.com/yasineken2002-sys/eken/pull/803)), födelsen ur ett godkänt förslag ([#807](https://github.com/yasineken2002-sys/eken/pull/807)) och läsytan `/delegationer` med återkalla/pausa/återuppta/förläng (PR 3). **Preferenserna saknar beslutshärkomst:** `AiMemory` har `AiMemoryType.preference` men varken `authorityKind`, `handlingAv` eller en pekare till beslutet posten föddes ur (mätt: noll träffar i `model AiMemory`), och det finns ingen yta där hyresvärden ser eller ändrar dem — bara `DELETE /ai/memory`. "Se vad systemet tror om hen" är alltså uppfyllt för BEFOGENHETER, inte för preferenser |
| 8 | Agentens frågor + observationslager + delegationsförslag | 7 | den frågar innan du frågar, och föreslår i stället för att ta sig rätt | **DELVIS** `ff8422e` — byggt: **torrläget** (PR 1), delegationsförslagens mätunderlag: kö + låst sveparcron med hjärtslag, tre domar (`WOULD_EXECUTE`/`NO_DELEGATION`/`BLOCKED`), domen på inkorgens kort och i modalen, och kolumnen "Skulle ha utlöst" på `/delegationer`. **Saknas:** agentens FRÅGOR — planens tre frågeslag och frågebudgeten har varken modell, kanal eller yta (mätt: noll träffar på en frågemodell i schemat), och **observationslagret** — `AiMemory` finns men fylls bara av en Haiku-extraktion ur chatten, saknar beslutshärkomst och läses UPP i systemprompten i stället för att sökas i, vilket Del 7 uttryckligen förbjuder. Ingenting räknar hyresvärdens beslutsmönster |
