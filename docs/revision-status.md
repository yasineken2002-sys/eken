...
| **Faktura-delbetalning auto-matchas inte** (M1)                                       | FINNS            | `apps/api/src/reconciliation/reconciliation.service.ts:847` och `:937` kräver belopp ≈ `invoice.total` — en delbetalning når aldrig `applyMatchToInvoice`                                                                                                                                                                                                                                   | ASYMMETRI, inte avsaknad av maskineri: den manuella vägen tillåter partial (`:2270`) och avi-vägens OCR-gren tillåter partial (`:886`). `allowPartial`-parametern finns redan (`:1271`). Kvar: släpp igenom partial på den DETERMINISTISKA fakturagrenen, behåll allt-eller-inget för fuzzy (D3) | medel                    | 2026-08-22 · `f3f47dc` |
