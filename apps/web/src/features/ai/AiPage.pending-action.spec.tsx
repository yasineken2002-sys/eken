...
vi.mock('./components/Composer', () => ({
  Composer: ({ value, onChange, onSubmit }: {
    value: string; onChange: (text: string) => void; onSubmit: () => void
  }) => <div>
...
vi.mock('./components/ConversationSidebar', () => ({
  ConversationSidebar: ({ onNewConversation, onSelect }: {
    onNewConversation: () => void; onSelect: (id: string) => void
  }) => <div>
...
vi.mock('./hooks/useAttachments', () => ({
  useAttachments: () => ({
    attachments: [], readyIds: [], blockingReason: null, clear: vi.fn(),
    remove: vi.fn(), add: vi.fn(), retry: vi.fn(),
  }),
}))
...
const conversation: AiConversation = {
  id: 'samtal-1', organizationId: 'org-1', userId: 'user-1', title: 'Fråga',
  createdAt: '2026-09-12T10:00:00Z', updatedAt: '2026-09-12T10:00:00Z',
  messages: [{
    id: 'user-message', conversationId: 'samtal-1', role: 'user',
    content: 'Kan du hjälpa mig?', createdAt: '2026-09-12T10:00:00Z',
  }],
}
...
beforeEach(() => {
  ...
  const body = new ReadableStream<Uint8Array>({ start(controller) { transport = controller } })
...
async function startQuestion() {
  render(<QueryClientProvider client={queryClient}><AiPage /></QueryClientProvider>)
...
describe('AiPage — bekräftelseförklaring genom den verkliga SSE-läsaren', () => {
  it.each(['samma läsning', 'delad JSON och UTF-8'])(
    ...
    async (mode) => {
      ...
      await deliver(mode === 'samma läsning' ? [bytes] : [
      ...
      expect(queryClient.getQueryData(['ai-conversation', action.conversationId])).toEqual(conversation)
      expect(screen.getAllByText(explanation)).toHaveLength(1)
  ...
  it('behåller förklaringen mellan två bekräftelser och rensar efter slutligt svar', async () => {
    api.confirmAction.mockResolvedValueOnce({
      conversationId: action.conversationId, reply: '',
    ...
    expect(api.confirmAction).toHaveBeenLastCalledWith({
      conversationId: action.conversationId, toolName: action.toolName,
      toolInput: action.toolInput, confirmed: true,
    })
  ...
  it.each(['Avbryt', 'Nytt samtal', 'Annat samtal'])(
    ...
    async (exit) => {
      ...
      if (exit === 'Avbryt') {
        expect(api.confirmAction).toHaveBeenCalledTimes(1)
        expect(api.confirmAction).toHaveBeenCalledWith({
          conversationId: action.conversationId, toolName: action.toolName,
          toolInput: action.toolInput, confirmed: false,
        })
      } else {
        expect(api.confirmAction).not.toHaveBeenCalled()
      }
