...
describe('Legal retrieval (Etapp 2, PR 2.2)', () => {
  ...
  describe('Uppmätt träffsäkerhet mot eval-setet (ärlig baslinje)', () => {
    it('BM25-retrieval når minst 14/26 answerable-fall (uppmätt baslinje)', async () => {
      const report = await runRetrievalEval()
      // Nämnaren är pinnad med avsikt: den som lägger till eval-fall ska tvingas
      // MÄTA om golvet nedan, inte se det tyst spädas ut. #406 la åtta
      // inkassokostnadsfall → 18 blev 26 (uppmätt 2026-08-11).
      expect(report.answerableTotal).toBe(26)
      // Uppmätt: 14/26 med BM25 (var 12/18 före #406; naiv substräng-räkning gav
      // 8/18). Av de åtta nya fallen träffar BM25 två — paminnelseavgift-lagens-
      // egna-ord och kravbrev-avgift, båda formulerade med lagens EGNA ord.
      // De sex övriga är #406:s vokabulärgap. Floor-assertion = regressionsspärr;
