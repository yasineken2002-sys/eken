 ...
 *
 * Därför ligger det BINDANDE zonprovet i `avisering.notice-render-date-tz.spec.ts`:
 * det renderar avin i barnprocesser med `TZ` satt i barnets miljö (UTC och
 * America/New_York), och en mätt kanariefågel vägrar datumet om barnets zon
 * eller offset inte är de begärda. Filen är egen för att jest-processen där
 * inte ska ladda tjänstens importgraf — bara barnet gör det.
 *
 * Fallen här (processens egen zon) är ett snabbprov och bevisar ingenting om
 * zonberoendet.
 */
