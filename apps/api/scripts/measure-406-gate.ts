...
// rör INTE fältet — mäts här som om det fanns, implementeras inte skarpt.
//
// TERMERNA ÄR SKRIVNA PER PARAGRAF, INTE PER EVAL-FRÅGA. Det är skillnaden
// mellan en mätning och en facitinjektion: 2 § OCH 4 § får båda ordet
// "påminnelseavgift" (2 § bär rätten, 4 § bär taket för samma avgift), så
// mätningen kan visa om mekanismen väljer RÄTT paragraf per fråga eller bara
// lyfter allt som råkar ha fått ordet.
...
const searchTermIndexText = (chunk: LegalChunk): string => {
  const terms = SEARCH_TERMS[legalChunkId(chunk)]
...
function assertBaselineParity(
  ...
  if (
    ...
    throw new Error('BASLINJEN MATCHAR INTE PRODUKTIONEN — mätningen är ogiltig, avbryter.')
  }
}
...
async function main(): Promise<void> {
  ...
  assertBaselineParity(indexes[0]!, cosineByCase, fusedByCase, semanticByCase)
  ...
  out()
  out(
    `- Baslinjens paritet mot produktionen: **max |Δscore| = 0, max |Δcoverage| = 0**, noll grindavvikelser (${LEGAL_EVAL_SET.length} frågor × ${chunks.length} chunkar).`,
  )
  out(
